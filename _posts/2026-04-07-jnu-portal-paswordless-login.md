...
그래서 패스키를 도입하면, 제한적이지만 비밀번호 없는 로그인을 위해서 휴대폰을 따로 꺼내지 않아도 됩니다. 우리 학교는 이클래스에 접근하기 위해서는 학교 전산 시스템에 로그인해야 합니다. 그래서 이번 변화로 수업 중간에 휴대폰을 사용하는 것을 학교에서 독려하는 형태가 되었다고 생각합니다. 최근에는 수업 중에 휴대폰을 꺼내 사용하는 것을 문제삼는 교수님이 많지 않다고 하더라도, 수업중에 이클래스에 접근하기 위해서라고 해도, 이것이 적절하다고 생각하지 않습니다.  
