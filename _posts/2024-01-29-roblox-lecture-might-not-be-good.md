...
특히 본격적으로 유니티를 가르치기엔 조금 어린 학생들을 대상으로 게임 제작 과정을 운영할 때 사용되었다. 유니티는 아직 비주얼 스크립팅 지원이 충분하지 않고, 블루프린트로 비주얼 스크립팅을 본격화한 언리얼 엔진은 저렴한 컴퓨팅 환경에서는 원활하게 동작하지 않기 때문이다.  

하지만 로블록스 수업을 직접 운영해보니, 로블록스보다는 다른 프로그램 스위트를 수업에 사용하거나, 수업을 관리 가능한 방향으로 제어할 수 있도록 추가적인 대책이 마련되어야 함을 느꼈다.  

## 로블록스를 컴퓨터 교육에 사용하는 것은 좋지 않을지도 모른다

### GUI 에디터만으로는 할 수 있는 것이 많지 않다

GUI 에디터로는 게임 세계에 물체나 이런 저런 요소를 배치하거나, 지형을 수정하거나 하는 것밖에 하지 못한다. 역시 로블록스에도 공식 비주얼 스크립팅 솔루션이 없어서 본격적으로 스크립트 수업을 들어가게 되면 수업의 난이도가 급격히 높아지게 되었다.  

![](/static/posts/2024-01-29-roblox-lecture-might-not-be-good/스크린샷%202024-01-29%20114433.png)  

최소한 커뮤니티 주도 무료 비주얼 스크립팅 솔루션이 괜찮은 것이 있으면 좋겠지만, 제대로 작동하는 것으로 보이는 것은 없었다. 그렇다고 물체 배치만 반복시키는 것은, 수업 시수가 지나치게 많이 남는 것도 문제지만, 게임을 제작하겠다는 의미로서의 수업이라고 보기에는 어려움이 있다.  

### Lua는 입문하기 좋은 프로그래밍 언어가 아니다

오브젝트 배치에서 벗어나 로블록스에서 스크립팅 언어로 채택한 Lua를 가르치기 시작해도 문제다.  

Lua는 이미 프로그래밍 언어를 알고 있는 사람에게는 편리한 문법을 가지고 있을 수 있지만 로블록스를 위해 프로그래밍에 새로 입문한 경우에는 느낌이 조금 다를 수 있다.  
...
물론 이렇게 상대적으로 어려운 내용을 문법의 일종으로서 취급하고 자세한 원리를 가르쳐주지 않은 채 수업을 계속해서 진행하는 것도 방법이다. 실제로 그러한 방법을 시도해보았는데, 물론 문법은 일반적으로 암기의 영역이라고 하더라도 꽤 많은 문법을 매 시간마다 반복해서 알려주어야 했다.  

반복해서 알려주는 것 자체는 문제되지 않지만, 학생들은 칠판에 빼곡히 적어놓은 문법과 그 사용법을 매번 일일이 찾아서 자신이 원하는 부분에 대입해야 했다. 대부분의 경우는 그렇게 해서는 적절히 동작하지 않으므로, 필요한 것을 일일이 찾는 데 들어가는 소모감과 반복해서 컴퓨터로부터 받는 코드 오류에서 오는 좌절이 수업 자체의 난이도와 흥미를 떨어뜨리는 요인이 되었다.  
...
수업 주제에 맞는 게임 세계를 만들지 않았다 정도의 문제라기보다, 매 수업마다 같은 일만 반복하면서 수업으로서의 의미가 퇴색되고 있는 수준이었다. 사실상 위에서 우려했던 "단순 물체 배치만 반복시키는 수업"과 다를게 없어진다.  
...
때문에 로블록스를 실제로 수업에 활용하려고 한다면, 그리고 교육적인 효과를 기대한다면, 이 수업을 통해 획득하고자 하는 바가 무엇인지 제대로 인식하고, 수업을 효과적으로 관리해낼 수 있는 방법과 수업을 진행할 수 있는 환경을 마련하는 대책이 필요하다.  
