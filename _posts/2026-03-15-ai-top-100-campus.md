...
- GitHub Copilot: 정말 프로그래밍에 파인튜닝한 것인지, 프로그래밍 성능에 느껴지는 것이 있어 프로그래밍 작업에 주로 사용하고 있다.  
- Cerebras: 모델을 하드웨어로 구현해 토큰을 고속 출력하는 프로바이더이다. 이 대회에서 사용할 일이 많을 것 같아서, 따로 크레딧을 구매해두었다.  
    <video src="/static/2026-03-15-ai-top-100-campus/cerebras.mp4">  
...
이들 공급자의 모델을 각각 Visual Studio Code(Github Copilot, Kilo Code), OpenClaw, 각 공급자가 제공하는 채팅 인터페이스를 통해 교차해 사용했다.  

![](/static/2026-03-15-ai-top-100-campus/devices.drawio.svg)
...
OpenClaw는 어떤 사고 과정을 거친 것인지, 로컬의 git credential을 제거해버려 파일 공유 파이프라인이 완전히 무력화되었다. 심지어 이 문제를 파악한 건 대회 절반이 가까워졌을 때였다.  

![](/static/2026-03-15-ai-top-100-campus/ocr.swift.png)  
...
![](/static/2026-03-15-ai-top-100-campus/poe-api.png)  
_(참고) 0pts 부분은 입출력 0의 오류가 아니라, 프로모션 때문인지 코스트가 0이어서 이렇게 되었다._

이윽고 OpenClaw는 특정한 모델을 사용하라는 명령을 모두 무시하고 Poe/GPT-5.3-Codex-Spark만 고집하는 상황에 이르렀다.  

문제 풀이 지시의 자동화 작업에 있어, OpenClaw는 가장 핵심의 역할이었다. 하지만 대회 중반부터는 더 이상 OpenClaw를 신뢰할 수 없게 되었다고 판단, 모든 지시를 직접 하는 것으로 바꾸었다.  

![](/static/2026-03-15-ai-top-100-campus/gemini-search-1.png)  
![](/static/2026-03-15-ai-top-100-campus/gemini-search-2.png)  
...
![](/static/2026-03-15-ai-top-100-campus/instagram-dm-1.png)  
