...
행렬 $A$의 역행렬을 구하기 위해 행렬식(Determinant)을 계산하면 다음과 같다. $A$가 $2 \times 2$ 행렬이므로 역행렬 공식을 쉽게 적용할 수 있다. 수식의 편의를 위해 행렬식 $\det(A)$를 $D$로 치환한다.
...
우변이 음수이므로 고유값 $\lambda$는 복소수(Complex number) 형태로 도출된다.
