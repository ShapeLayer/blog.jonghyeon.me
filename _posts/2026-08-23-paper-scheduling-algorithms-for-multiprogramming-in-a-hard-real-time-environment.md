...
논문이 발표된 1970년대 전후, 산업 공정에 제어/감시 목적으로 컴퓨터가 널리 활용되기 시작했다. 이러한 적용 방법은 이전과는 비교해서 새로운 것이었는데, 초기의 컴퓨터는 주로 계산을 수행하기 위해 배치 처리에 사용되어, Time-critical한 요구사항이 발생하는 경우가 거의 없었기 때문이다. 하지만 산업 공정에서는 실시간으로 공정을 제어하고 감시해야 하므로, 컴퓨터에게 시간 측면에서 높은 신뢰성이 요구되었다.  
...
(A5) 시스템에서 발생하는 비 주기적인 작업은 초기화, 장애 복구 등 특수한 경우로 한정한다. 이 작업들은 실행되는 동안 주기적인 작업을 대체하지만, 이 작업 자체로는 엄격한 마감 기한을 갖지 않는다.  
...
두 개의 작업에 대해 rate-monotonic 우선순위 할당이 최적임을 확인했으므로, 이를 일반화하면 다음과 같이 정리할 수 있다.  
...
> **정리 4**  
> 고정 우선순위를 가지며, 임의의 두 요청 주기 간 비율이 2 미만인 $m$ 개의 작업 집합에 대해, 프로세서 이용률의 최소 상한은 $U = m(2^{1/m} - 1)$ 이다.
...
이 식을 각 $g_j$ 에 대해 편미분하여 0으로 두면 연쇄적인 차분방정식이 얻어지고, 이를 풀면

$$
g_j = 2^{(m-j)/m} - 1,\qquad j = 0, 1, \dots, m-1
$$

이라는 일반해를 얻는다. 이를 대입하면
...
> **정리 5**  
> 고정 우선순위를 갖는 $m$ 개의 작업 집합에 대해, 프로세서 이용률의 최소 상한은 (주기 비율에 대한 제약 없이) $U = m(2^{1/m} - 1)$ 이다.
...
> **정리 7**  
> 주어진 $m$ 개의 작업 집합에 대해, deadline-driven 스케줄링 알고리즘이 실행 가능하기 위한 필요충분조건은 다음과 같다.
> $$
