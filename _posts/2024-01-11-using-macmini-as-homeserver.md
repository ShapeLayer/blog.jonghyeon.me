...
맥의 시스템 커널에서 49일 17시간 2분 47초를 연속해서 가동하면 TCP 네트워킹이 완전히 마비되는 버그가 발견되었다. [#](https://news.hada.io/topic?id=28312) 경험적으로 종종 어딘가 이상이 있음을 느끼고는 있었어도, 환경을 잘못 구성해서 개인적인 문제인 줄 알았었다. (그리고 내게 있어서는 아마 실제로 환경을 잘못 구성해서 시스템 실패가 난 상황이 더 많았을 것이다.)  

하지만 체감상 49일이 지난 뒤에도 네트워킹에 문제가 있었던 것 같지는 않다. TCP가 아닌 UDP나 다른 네트워킹 구현을 사용하고 있었던 건지, 어딘가 잘 안될 때마다 시스템을 재부팅해서 실제로 이 버그 사례를 제대로 인식하지 못한 것인지는 잘 모르겠다.
